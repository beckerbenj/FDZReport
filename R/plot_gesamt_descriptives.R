...
#'
#'@export
plot_gesamt_descriptives <- function(gesamt_descriptives, ylab, ylim = c(NA_integer_, NA_integer_)) {
  by_var <- names(gesamt_descriptives)[1]
  ggplot2::ggplot(data = gesamt_descriptives, mapping = ggplot2::aes(x = .data[[by_var]],
                                                   y = .data$value, color = .data$Statistik,
                                                   group = .data$Statistik)) +
    ggplot2::geom_line(linewidth = 1)  +
    ggplot2::ylab(ylab) +
    ggplot2::geom_text(mapping = ggplot2::aes(label = .data$value), hjust= 0.5, vjust = 2,
    ...
    ggplot2::scale_y_continuous(breaks = seq(0, 800, by = 100), limits = ylim)
}
